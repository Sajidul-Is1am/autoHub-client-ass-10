                ...
                <div className="navbar w-full">
                    ...
                    <div className="navbar-end">
                        <div className="dropdown ">
                            <label tabIndex={0} className="btn btn-ghost lg:hidden">
                                {
                                    user ? <img className="w-10 h-10 rounded-full" src={user.photoURL} /> : <NavLink className={"btn"} to={'/login'}>Login</NavLink>
                                }
                            </label>
                            <ul tabIndex={0} className="menu-sm dropdown-content mt-8  z-[1] p-4 shadow bg-[#08213e] border  w-36 text-center -ml-10">
                                {
                                    user ?
                                        <>
                                            <p className="mx-2 mb-4">{user.displayName}</p>
                                            <NavLink className={"p-4 text-red-500"} onClick={LogOut} to={'/login'}>LogOut</NavLink>
                                        </> : ''
                                }
                            </ul>
                        </div>
